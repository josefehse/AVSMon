...
#####    EVERY QUERY SHOULD HAVE THE NSX-    VCENTER-     HCX-   so that we know what the functions do   #####

# Azure Powershell
# Get diagnostics enabled
# Get all the private clouds first, then iterate through to show if they have diagnostics enables
Get-AzVmwarePrivateCloud
$privateCloudID = "/subscriptions/bd4d88c1-fc0f-482f-b57b-2f3ed541945e/resourceGroups/AVS-PrivateCloud/providers/Microsoft.AVS/privateClouds/AVS-sddc"
Get-AzDiagnosticSetting -ResourceId $privateCloudId
$PSVersionTable
# returns either enabled or returns nothing.
